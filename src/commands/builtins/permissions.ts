...
function listShellPolicy(ctx: Parameters<Command["handler"]>[1]): void {
	...
	ctx.emit(
		[
			...
			"  hard blocks: rm -rf / or $HOME, rm -rf /*, fork bombs, mkfs, raw block-device writes, and curl|sh or wget|sh.",
			"  warnings: sudo, chmod 777/a+w, git push --force, and broad parent-directory deletes.",
